...
class Theme(Builder):
    ...
    def generate_sources(self):
        ...
        repo = self.srvbes.get_repo_dict()
        source_dir = repo['source']
        self.log.error(f"Sources: {source_dir}")
        import glob
        source_files = glob.glob(os.path.join(source_dir, '*.adoc'))
        for filepath in source_files:
            ...
            for line in text:
                ...
                n += 1
            body = ''.join(text[n+1:])
            fname = filepath.replace('.adoc', '_body.adoc')
            with open(fname, 'w') as fout:
                fout.write(body)
            self.log.error(f"Created body file {fname}")
