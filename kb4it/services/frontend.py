...
class Frontend(Service):
    ...
    def theme_list(self):
        ...
        for dirname in global_themes:
            try:
                ...
                self.log.info(f"[FRONTEND] - [{n}] - (G) Theme '{self.runtime['theme']['id']}' ({self.runtime['theme']['description']})")
                n += 1
        ...
        self.log.debug("[FRONTEND] - 2) Search themes installed locally (%s)", ENV['LPATH']['THEMES'])
    ...
    def theme_load(self, theme_name=None):
        ...
        if not os.path.exists(theme_conf):
            ...
            try:
                ...
                self.log.debug("[FRONTEND] - \tTheme '%s' loaded successfully", self.runtime['theme']['id'])
            except Exception as error:
                self.log.error("[FRONTEND] - \tTheme '%s' couldn't be loaded", self.runtime['theme']['id'])
    ...
    def theme_search(self, theme=None):
        ...
        if theme is None:
            ...
            self.log.debug(f"[FRONTEND] - \tFound directory for theme: '{theme}'")
